# Tareas — n_mail_watch.sh

## Fase 1 a 3: Definición

- [x] Task 1 - Crear directorios `specs/`, `scripts/` y `tests/` dentro de `n_mail_watch/`.
...
- [x] Task 5 - Crear `scripts/n_mail_watch.sh` a partir del patrón del repositorio.
...
- [x] Task 8 - Validar patrón de log, umbral, destinatarios, directorio y archivos resueltos (AC-011, AC-012).
- [x] Task 9 - Implementar detección de backend de mail `sendmail`/`mailx` (AC-012).
- [x] Task 10 - Implementar conteo de mails enviados del día actual por `fecha + cuenta` usando la misma regla de `cantidad_mail_enviados` (AC-004, AC-005).
- [x] Task 11 - Implementar filtro por umbral y envío de un mail por cuenta excedida (AC-006, AC-007, AC-008, AC-009, AC-010).
- [x] Task 12 - Otorgar permisos de ejecución a `scripts/n_mail_watch.sh`.

## Fase 5: Verificación y Tests

- [x] Task 13 - Crear `tests/test_n_mail_watch.bats`.
...
- [x] Task 18 - Ejecutar `shellcheck` sobre `scripts/n_mail_watch.sh`.
- [x] Task 19 - Ejecutar `./ci/run_checks.sh -p n_mail_watch`.

## Fase 6: Documentación

- [x] Task 20 - Crear `n_mail_watch/Readme.md` con descripción, flags, ejemplos y cron.
